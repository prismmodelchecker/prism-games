// DTMC model implementing collective decision making algorithm of:
	...
	[] true ->	  0 : true
		// - exploring with probability Pexp
			...
			// -- evaluating site and changing preference with probability Pswitchxy
			  + Pexp/K * Pswitch#i#_#j# : (location#i#'=#j#) & (preference#i#'=#j#) & (confidence#i#'=1)
			  + Pexp/K * (1-Pswitch#i#_#j#) : (location#i#'=#j#)
		...
		// - communicating with other agents in the same site with probability 1-Pexp
			...
			// -- trying to communicate with agent
			  + (1-Pexp) * Pmeet_p#i# * (location#i#=location#j#?1:0) * (preference#i#=preference#j#?1:0) : (confidence#i#'=inc_conf#i#) & (confidence#j#'=inc_conf#j#) // same site
			  + (1-Pexp) * Pmeet_p#i# * (location#i#=location#j#?1:0) * (preference#i#=preference#j#?0:1) * Pwin#i#_#j# : (confidence#i#'=inc_conf#i#) & (preference#j#'=preference#i#) & (confidence#j#'=1) // win
			  + (1-Pexp) * Pmeet_p#i# * (location#i#=location#j#?1:0) * (preference#i#=preference#j#?0:1) * (1-Pwin#i#_#j#) : (confidence#i#'=1) & (preference#i#'=preference#j#) & (confidence#j#'=inc_conf#j#) // lose
