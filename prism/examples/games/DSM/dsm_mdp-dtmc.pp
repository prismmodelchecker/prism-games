...
module player#i#
	...
	[] active = 0 & job#i# > 0 & time < max_time -> (job1'=new_j1)#for j=2:N# & (job#j#'=new_j#j#)#end# & (time'=time+1);

	// initiate the job with probability P_init
	[] active = 0 & job#i# = 0 & time < max_time -> P_init*P_J1 : (job_arrived#i#'=1)
				       			    #for j=2:L#
							    + P_init*P_J#j# : (job_arrived#i#'=#j#)
							    #end#
				       			    + (1-P_init) : (job1'=new_j1)#for j=2:N# & (job#j#'=new_j#j#)#end# & (time'=time+1);

	// start job if cost below the limit
	[] job_arrived#i# > 0 & cost <= cost_limit & time < max_time -> (job#i#'=job_arrived#i#) #for j=1:i-1# & (job#j#'=new_j#j#)#end##for j=i+1:N# & (job#j#'=new_j#j#)#end# & (job_arrived#i#'=0) & (time'=time+1);

	// back-off with probability 1-P_start
	[] job_arrived#i# > 0 & cost > cost_limit & time < max_time ->   P_start : (job#i#'=job_arrived#i#) #for j=1:i-1# & (job#j#'=new_j#j#)#end##for j=i+1:N# & (job#j#'=new_j#j#)#end# & (job_arrived#i#'=0) & (time'=time+1)
						   + (1-P_start) : (job1'=new_j1)#for j=2:N# & (job#j#'=new_j#j#)#end# & (job_arrived#i#'=0) & (time'=time+1);
	// finished
	[] time=max_time -> (job1'=new_j1)#for j=2:N# & (job#j#'=new_j#j#)#end#;
...
formula cost = job1>0?1:0 #for i=2:N# + job#i#>0?1:0#end#;
