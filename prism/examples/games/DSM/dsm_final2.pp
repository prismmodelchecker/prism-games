...
module player#i#
	...
	[] sched=#i# & job_arrived#i# > 0 & price <= price_limit & time < max_time -> (job#i#'=job_arrived#i#) #for j=1:i-1# & (job#j#'=new_j#j#)#end##for j=i+1:N# & (job#j#'=new_j#j#)#end# & (job_arrived#i#'=0) & (time'=time+1) & (sched'=0);

	// back-off with probability 1-P_start
	[] sched=#i# & job_arrived#i# > 0 & price > price_limit & time < max_time ->   P_start : (job#i#'=job_arrived#i#) #for j=1:i-1# & (job#j#'=new_j#j#)#end##for j=i+1:N# & (job#j#'=new_j#j#)#end# & (job_arrived#i#'=0) & (time'=time+1) & (sched'=0)
