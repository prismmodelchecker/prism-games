...
All satisfy constraint $\forall i. |R_{a_i}|=1 \wedge   \forall i.1 \le|\{ R_{a_j} : r_i \in R_{a_j} (1 \le j \le 5 )\}|\le 2$.}
\label{tab:optimal_r}
\end{table}
\vspace{-30pt}
...
PCTL formulae with the agent coalitions. The following formulae were used to obtain the results presented in this section:
\vspace{-5pt}
\begin{itemize}
...
This is interesting, because it suggests that having strong connectivity within the team outweighs the exposure to many hostile agents. Performance of $O_r$ is the most consistent as the maximum reward increases steadily with the coalition size.
