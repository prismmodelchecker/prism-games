...
\caption{Model checking results for agent organisations from figure \ref{fig:network_configurations} with optimal resource allocations from table \ref{tab:optimal_r} for offline version of the Algorithm \ref{alg:main_process}. Tables also show largest and smallest individual agent rewards. }
\end{table}
...
\caption{Model checking results for agent organisations from figure \ref{fig:network_configurations} with optimal resource allocations from table \ref{tab:optimal_r} for online version of the Algorithm \ref{alg:main_process}. Tables also show largest and smallest individual agent rewards.  }
\end{table}
...
\caption{Task completion probabilities for optimal agent organisations using Algorithm \ref{alg:join_team_org}'s offline version (see Algorithm \ref{alg:main_process}). Results obtained by model checking PCTL formulae on DTMC model.}
\end{table}
...
\begin{table}
 ...
 \begin{tabular}{ | l | l | l | l |}
    ...
    $O_r$ & 0.65941 & 0.0.65941 & 0.36712 \\ \hline
    $O_s$ & 0.47202 & 0.47202 & 0.07465 \\ \hline
    $O_{ia}$ & 0.62780 & 0.62780 & 0.29270 \\ \hline
\end{tabular}

\caption{Task completion probabilities for optimal agent organisations using Algorithm \ref{alg:join_team_org}'s online version (see Algorithm \ref{alg:main_process}). Results obtained by model checking PCTL formulae on DTMC model.}
\end{table}
