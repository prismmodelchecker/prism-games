...
\caption{Experimental configurations of the agent organisations with optimal resource allocation (see Tab.~\ref{tab:optimal_r}). In parentheses is the abreviation that we will use to refer to the organisation throughout this paper.}
...
which is modelled as an MDP. Using PRISM we find the maximum expected rewards and task completion probabilities for all agent organisations and compare the results with a specific strategy used in Alg.~\ref{alg:join_team_org}. Due to the size limitations we do not present actual strategies here.\footnote{The instructions on how to generate optimal strategies for MDPs can be found in ...}.
...
\caption{Maximum task completion probabilities for optimal agent organisations using Alg.~\ref{alg:join_team_nondet}'s online and offline versions (see Alg.~\ref{alg:main_process}). Results obtained by model checking PCTL formulae on MDP model.}
\label{tab:mdp_probs}
\end{table}

In Tab.~\ref{tab:mdp_probs} we can see the maximum expected organisation rewards that can achieved by all agents collaborating. All organisations except $O_s$ can make sure that at least one task is completed with probability $1.0$, no matter what the scheduling is. The following PCTL formulae were used to get the results:
\begin{itemize}
...
Fig.~\ref{fig:bar_chart_mdp} compares maximum expected rewards with a specific strategy used in the original algorithm, for all agent organisations in both offline and online settings. More significant improvement can be obtained for the offline version of the algorithm for both reward types than for online version. This is result shows that there is more potential for collaboration for agents in offline version. Small performance improvement for online version suggests that original strategy of Alg. \ref{alg:join_team_org} is close to optimal when teams are formed before tasks are advertised.  
...
by considering all agents in the coalitions as one player, and all the other as the second one. 
...
``what is the expected probability for coalition C to complete task $T_1$/$T_2$ when agents $A\setminus C$ are hostile?'',
 \item $[C]{\tt P}_{max,min=?}[\,\Diamond\,T_{1} \wedge T_2 \mathit{\ completed}\,]$ -
``what is the expected probability for coaltion C to complete both tasks $T_1$ and $T_2$ when agents $A\setminus C$ are hostile?'',
\end{itemize}

And for rewards: 
\begin{itemize}
 \item $[C]{\tt R}_{max,min=?}^{W_{1}(O)}[\,\Diamond\,\mathit{finished}\,]$ -
``what is the maximum expected total reward $W_1$ for coalition C when agents $A\setminus C$ are hostile?'',
 \item $[C]{\tt R}_{max,min=?}^{W_{2}(O)}[\,\Diamond\,\mathit{finished}\,]$ -
``what is the maximum expected total reward $W_2$ for coalition C when agents $A\setminus C$ are hostile?'',
\end{itemize}
...
This result stresses the importance of positions in the network and resources held by the agents. 
For example agent $a_4$ is in all optimal coalitions of sizes greater than 1 for $O_{ia}$ agent organisation.
This is because it is the most connected agent, and the only one having access to agent $a_5$ which is isolated.
For organisation $O_r$, however, which agents belong to optimal coalition varies with coalition size.
...
Fig. \ref{fig:coalition_performance} shows how agent organisations comprare in term of maximum performance of different size coalitions. Organisation $O_{fc}$ outperforms all the other in all examples, this is an interesting result which shows that having strong connectivity within the team is potentially more important than being connected to many hostile agents. Performance of $O_r$ is the most consistent as maximum reward increases steadily with coalition size, however graphs also show that in order to be as effective as more connected networks like $O_{fc}$, coalition has to be formed of most agent in the newtworks. Performance of $O_s$ against $O_r$ for coalition sizes up to 3 illustrates the importance of having a heavily interconnected agent for small coalitions.


Important difference between online and offline settings can be seen for reward $W_1$ in Fig.\ref{fig:off_w1} and \ref{fig:on_w1}. The big jump in difference can be seen when going from coalition size 2 to 3, especially, for strongly connected $O_{fc}$ and $O_{ia}$, as coalitions can make sure that one task is completed almost surely (see Tab. \ref{tab:optimal_1task}0 thus asuring reward 3 for coalition.
