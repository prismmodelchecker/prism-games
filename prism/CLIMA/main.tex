...
motivates agent to be in a successful team. From the organisation's perspective $W_1$ reward
shall be used when resources are limited, whereas $W_2$ reward will encourage agents to introduce
...
has no other agents committed to it, the agent can choose to initiate a team, and does so with a probability given in Eqn.~\eqref{eq:init_prob} (i.e.,
...
In Alg.~\ref{alg:join_team_org} we reproduce pseudocodes of the \textsc{JoinTeam} algorithm introduced in \cite{gaston2005agent}. (It combines team initiation and team joining.) %\textsc{JoinTeam} algorithm is executed separately on each agent whenever it is called.
...
\begin{itemize}
 ...
 \item Replace probabilistic choice to initiate the team by nondeterministic choice;
...
\label{sec:results}
\vspace{-10pt}
In this section we present results obtained using three models: DTMC, MDP, and STPG. Tab.~\ref{tab:model_sizes} compares model construction information for different sizes of fully connected agent organisations\footnote{We choose fully connected agent organisation because it produces the largest models.}. All experiments are performed on a 2.8GHz Intel Core 2 PC, 4Gb of RAM running Fedora Core 13 operating system. Nondeterministic models have a relatively smaller state space because agent choices do not have to be resolved at the model construction stage. However, the model checking is generally more time consuming for MDPs and STPGs than for DTMCs. The time needed to verify PCTL formulae that we used is in the range of 5-240 seconds.
 \vspace{-20pt}
\begin{table}
\centering
\subfloat[DTMC]{
...
\caption{Model comparison for different number of agents in a fully connected agent organisation for offline version of the Alg.~\ref{alg:join_team_org}.}
\label{tab:model_sizes}
\end{table}
\vspace{-20pt}
As mentioned in Sec.~\ref{subsec:exp_set}, for each topology from Fig.~\ref{fig:network_configurations} we obtain optimal resource allocations using PCTL model checking on the DTMC model of the offline version of the algorithm (see Alg.~\ref{alg:main_process} and Alg.~\ref{alg:join_team_org}). The following PCTL formulae are used to compute the expected rewards of the agent organisation under a particular resource allocation:
\vspace{-3pt}
\begin{itemize}
...
\vspace{-10pt}
In this section we presentresults of model checking the DTMC model of Alg.~\ref{alg:join_team_org}. We consider four experimental agent organisations from Fig.~\ref{fig:network_configurations}, as well as offline and online versions of the algorithm (see Alg.~\ref{alg:main_process} for details). To discuss the results, we compare the overall performance of agent organisations and consider the disparity between the worst and best individually performing agents within them. Then we move on to discuss the effects of network topologies and offline vs online versions of the algorithm on the aforementioned characteristics. Finally, we discuss several interpretations of reward structures and argue why it is important to consider different ones to get a better understanding of the system.
...
An attentive reader would have noticed that the online version produces larger expected $W_1$, but smaller expected $W_2$ rewards. This observation shows that in an online version the algorithm organises agents into teams, which increases the expectation for more agents to be in a successful team, but this decreases the expected total number of tasks completed. These are summarised in Tab.~\ref{tab:task_compl_dtmc} where the task completion probabilities are presented. The following PCTL formulae were used to find the probabilities using PRISM:
\vspace{-5pt}
\begin{itemize}
...
Tab.~\ref{tab:mdp_rewards} shows the maximum expected organisation rewards that can be achieved by all agents collaborating. It is not very difficult to see that $O_{fc}$, $O_r$ and $O_{ia}$ have the same maximum reward, no matter whether $W_1$/$W_2$ reward or online/offline version is taken, which outperforms the star organization $O_s$ in all circumstances. The PCTL formulae used to find the rewards in PRISM were the following:
\begin{itemize}
...
\vspace{-10pt}
\caption{Bar charts compare offline and online versions of Alg.~\ref{alg:join_team_org} analysed as a DTMC with the best-case performance of Alg.~\ref{alg:join_team_nondet} analysed as an MDP. }
...
``what is the expected probability for coaltion C to complete both tasks $T_1$ and $T_2$ when agents in $A\setminus C$ are hostile?'',
 \item $[C]{\tt R}_{max,min=?}^{W_{\{1,2\}}(O)}[\,\Diamond\,\mathit{finished}\,]$ -
``what is the maximum expected total reward $W_1$/$W_2$ for coalition C when agents in $A\setminus C$ are hostile?'',
\end{itemize}
\vspace{-35pt}
...
\vspace{-8pt}
\caption{Graphs compare how the optimal coalition performance depends on its size. For precise data see Tab. \ref{tab:opt_coa_reward_offline}-\ref{tab:opt_coa_reward_online} in Appendix \ref{app:coalitions_data}.}
...
%
For all agent organisations from Fig.~\ref{fig:network_configurations} we enumerate all possible coalitions with different sizes and PRISM returns task completion probabilities (one task or both tasks) and rewards obtained ($W_1$ or $W_2$). These are done for both online and offline versions of the algorithm. It turns out that there exist coalitions of all sizes that are optimal with respect to all evaluated criteria; they are shown in Tab.~\ref{tab:optimal_coalitions}.
...
\end{table}
\vspace{-30pt}
Fig.~\ref{fig:coalition_performance} shows a comparison of agent organisations in terms of the maximum performance for different coalition sizes. $O_{fc}$ outperforms others in all examples. This is interesting, because it suggests that having strong connectivity within the team outweighs the exposure to many hostile agents. Performance of $O_r$ is the most consistent, as the maximum reward increases steadily with the coalition size.
...
\vspace{-8pt}
In this paper, using a team formation algorithm as a case study, we illustrated the usage of formal analysis techniques, namely, probabilistic model-checking for understanding and evaluating the performance of algorithms for multi-agent systems. We used three models, DTMC, MDP and STPG for the analysis and compared their capabilities for exploring different scenarios. Using a DTMC we analysed the performance of the original protocol. Then we extended the algorithm by allowing agents to make decisions nondeterministically, and using MDP model checking we obtained the best-case performance of the protocol, which allowed us to see how the original algorithm compares with the optimal one. To be able to perform further analysis of the coalitional setting we extended PRISM to support PCTL model checking of STPGs, and then discussed the results of the algorithm's performance in the competitive setting with hostile agents. We believe that there is a lot of potential in applying formal analysis to multi-agent systems, and this case study will serve as a motivating starting point for further work in this area.
...
E. Bonzon, M.-C. Lagasquie-Schiex, and J\'er\^ome Lang.
\newblock Efficient coalitions in {B}oolean games.
\newblock In {\em Texts in Logic
  and Games}, vol.5, chapter New Perspectives on Games and Interaction,
  pp. 293--297. 2008.

\bibitem{chatterjee2007strategy}
K.~Chatterjee, T.~Henzinger, and N.~Piterman.
\newblock {Strategy logic}.
\newblock {\em CONCUR 2007--Concurrency Theory}, pp. 59--73, 2007.
...
\newblock In {\em Proc. of AAMAS'08}, pp. 1015--1022. 2008.
...
\newblock In M.~Bernardo and J.~Hillston, editors, {\em Formal Methods for the
  Design of Computer, Communication and Software Systems: Performance
  Evaluation (SFM'07)}, volume 4486 of {\em LNCS (Tutorial Volume)}, pages
...
\newblock In {\em Proc. of TACAS'06}, pp. 450--454, 2006.
