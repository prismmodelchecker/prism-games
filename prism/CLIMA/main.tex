...
\aistis{This is were we say why verification of such algorithm is important event though it is simple. This also makes it different from traditional task allocation algorithms?}
...
\aistis{Here we should explain where the probabilties appear in the algorithm as well to motivate use of PMC (online vs offline).}
...
\aistis{We should put emphasis on what was new in this paper. Namely implementation and usage of games for competitive automated analysis of team formation protocol. }
...
\end{itemize}
%\vspace{-5pt}
\aistis{Add a paragraph for addressing 'implementation' of hostile agents, that is considering their minimising strategies in the game, emphasizing that this provides upper bound for the hostile behaviour. }
...
%\vspace{-10mm}
\aistis{Cross reference Tab. \ref{tab:dtmc_results} with the bar chart in Fig. \ref{fig:bar_chart_dtmc} }
\begin{table} [h]
...
\label{subsec:MDP}
\aistis{We could add a couple of sentences on sythesis of optimal strategies and how they are used to benchmark other strategies (we mention this already but should emphasize more).}
%\vspace{-2pt}
...
\end{itemize}
\aistis{get rid of Tab. \ref{tab:mdp_rewards} completely as the data here is the same as in the bar chart in Fig. \ref{fig:bar_chart_mdp} }
%\vspace{-30pt}
\begin{table}[h]
\centering
\subfloat[Offline]{
...
\aistis{Add positive description - "the designer could choose to implement this strategy``}
...
\label{subsec:STPG}
%\vspace{-5pt}
\aistis{In this section we could elaborate a bit more on hostility and synthesis of strategies.}
