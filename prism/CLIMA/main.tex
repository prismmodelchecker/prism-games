...
\aistis{Added a paragraph motivating probabilistic model-checking}
As is typical for multi-agent algorithms, probabilities play a crucial role in team formation protocols. % several aspects of the algorithm are modelled using probabilities. 
Firstly, agents are scheduled to act in a random order, following approach of
\cite{gaston2005agent}; secondly, in our setting, tasks are drawn from a task pool, following some probability distribution. This is particularly interesting in the case of \textsc{Online} version of the algorithm (see Alg.~\ref{alg:main_process}) where agents have to choose strategies to optimise against a set of tasks governed by certain probability distribution rather than a particular task. Finally, probabilities are used to implement strategies of agents themselves. These compel the use of \emph{probabilistic verification} techniques.
...
\aistis{Re-ordered the contributions stressing the extension to STPGs and coalitional analysis. }
...
%%\vspace{-9pt}
\noindent\emph{Related work.} Cooperative behaviour, which is one of the greatest advantages of agent-based computing, has been studied from many different angles over the years.
...
%\vspace{-6mm}
\aistis{Corrected typo's in the algorithms.}
\begin{algorithm}[h]
...
\Procedure{JoinTeam}{$a$, $T$, $M$}
  \ForAll {$T_i \in T$ in random order}
	...
	\If {$R_a \cap T_i \setminus R_{M_i} \neq \emptyset$} \Comment{agent has a missing resource (replaced by \textbf{true} if called from \textsc{Online})}
...
\footnote{See {\scriptsize\url{http://www.prismmodelchecker.org/files/clima11/}}.}.
...
%(for the example in Fig.~\ref{fig:prism_example}, built as an MDP, see
...
\centering
\scalebox{0.54}{
\subfloat[Fully connected ($\mathbf{O_{\mathit{fc}}}$)]{
  \begin{tikzpicture}
...
\subfloat[Isolated agent ($\mathbf{O_{\it ia}}$)]{
  \begin{tikzpicture}
      \tikzstyle{every node}=[draw,circle,fill=white,minimum size=5pt,
			      inner sep=0pt]
      \draw (0,0) 	 node (agent1) [label=90:$\LD a_1\ r_1 \RD$] {}
...
%\vspace{-10mm}
\aistis{Cross referenced table with bar-chart. }
\begin{table} [h]
...
``what is the probability to complete  both tasks?''. % $T_1$ and $T_2$?''.
\end{itemize}
%\vspace{-35pt}
\begin{table}[h]
 \centering
...
\label{subsec:MDP}
\aistis{Added a note about synthesis of optimal strategies. See more in last paragraph of the section.}
%\vspace{-2pt}
In this section, we present the analysis of Alg.~\ref{alg:join_team_nondet}, which is modelled as an MDP. Using PRISM we find the maximum expected rewards and task completion probabilities for all agent organisations and compare the results with the strategy used in Alg.~\ref{alg:join_team_org}. This is achieved by synthesizing the optimal strategy for MDP using PRISM and then model-checking the formulae on the resulting DTMC. Due to space limitations we do not present the actual strategies here.\footnote{The instructions on how to generate optimal strategies for MDPs can be found at:\\ {\scriptsize\url{http://www.prismmodelchecker.org/dev/manual/RunningPRISM/Adversaries.html}}}.
%\vspace{-28pt}
\begin{table}[h]
\centering
\subfloat[Offline]{
...
``what is the maximum probability to complete  both tasks?''. % $T_1$ and $T_2$?''.
\end{itemize}
%\vspace{-2pt}
\aistis{Removed one table which duplicated data in the bar chart. Restructured the analysis part. }
%\vspace{-30pt}
%\begin{table}[h]
%\centering
%\subfloat[Offline]{
...
\aistis{Added positive note to the strategy synthesis.}
...
\label{subsec:STPG}
%\vspace{-5pt}
\aistis{Changed the first paragraph to contain more on hostility of the agents (their ``implementation'') and synthesis of optimal strategies for hostile and coop agents. }
...
by considering each class of agents (cooperative or hostile) as a separate player in a zero-sum game. Cooperative agents fully cooperate to act in the interest of the organisation, i.e., maximising rewards $W_1$ and $W_2$, whereas all hostile agents together take actions so that to minimise the expected rewards gained by the organisation. As a result of solving STPG we are able to obtain (synthesize) the optimal strategies for both cooperative and hostile agents, these can be used in further design of the system. Here we present the expected rewards achieved by the optimal strategies, but due to space limitations we omit strategies themselves.
...
%For precise data, see Tab.
%\ref{tab:opt_coa_reward_offline}-\ref{tab:opt_coa_reward_online} in
%Appendix \ref{app:coalitions_data}.
}
...
%\vspace{-8pt}
%\aistis{Remove appendices altogether.}
%\begin{itemize}
...
%  \centering
%  \vspace{-60pt}
%	\includegraphics[scale=0.81]{images/transition_system}
%	\vspace{-35pt}
%	\caption{State transition system of MDP model from Figure \ref{fig:prism_example} generated by PRISM. Transitions are labelled with action names and probabilities. The state is labelled with values of variables -- each tuple is $(turn,num\_tasks,team1,team2)$.}
%\end{figure}
%\vspace{-60pt}
%
%\clearpage
