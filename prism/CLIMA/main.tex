...
\vspace{-10pt}
In this section we present the results of model checking the DTMC model of Alg.~\ref{alg:join_team_org} for experimental agent organisations from Fig.~\ref{fig:network_configurations}, as well as offline and online versions of the algorithm (see Alg.~\ref{alg:main_process} for details).
...
Important differences between the online and offline settings can be seen for reward $W_1$ in Fig.~\ref{fig:off_w1} and \ref{fig:on_w1}. When going from coalition size 2 to 3, especially, for strongly connected $O_{fc}$ and $O_{ia}$, coalitions can make sure that one task is completed almost surely (see Tab.~\ref{tab:optimal_1task}), and thus guaranteeing reward of at least 3 for the coalition, which results in a jump of performance.
...
\vspace{-8pt}
In this paper, using a team formation algorithm as a case study, we illustrated the usage of formal analysis techniques, namely, probabilistic model-checking for understanding and evaluating the performance of algorithms for multi-agent systems. We believe that there is a lot of potential in applying formal analysis to multi-agent systems, and this case study will serve as a motivating starting point for further work in this area.

As an ongoing and future work we plan to equip the analysis with abstraction techniques to allow for model checking larger systems. We also plan to explore parallel execution of the \textsc{JoinTeam} algorithm for multiple agents, as here we only considered sequential execution, and consider the problem of synthesising the optimal agent organisations for a given set of tasks.
