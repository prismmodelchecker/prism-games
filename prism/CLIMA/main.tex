...
In this paper, we use the PRISM probabilistic model checking tool \cite{KNP11}
to analyse various agent organisations for the team formation protocol of \cite{gaston2005agent}.
...
\taolue{online v.s. offline dichotomy already exists for DTMC...}
...
\comment{TL: might explain the intuition of these two rewards.}
...
\caption{Experimental configurations of the agent organisations with optimal resource allocation (see Tab.~\ref{tab:optimal_r}).}
...
In Tab.~\ref{tab:mdp_probs} we can see the maximum expected organisation rewards that can achieved by all agents collaborating. All organisations except $O_s$ can make sure that at least one task is completed with probability $1.0$, no matter what the scheduling is.
...
%Let us note that the strategy of an agent does not change when switching from offline to online setting, but it is the same strategy and the removal of constraint that the agent can only join a team for a task that it has the required skill for (which is obviously not known before the task is advertised) that results more agents joining teams. In the next section we will explore how the performance can be improved by changing the strategy of the agents so that they collaborate to optimise performance of the organisation.
%
We enumerate all the possible coalitions with different sizes and in each case, PRISM model checker returns task completion probabilities (one task or both tasks). This is done for both online and offline versions of the algorithm. Tab.~\ref{tab:optimal_coalitions} shows the optimal coalitions with different sizes. Tab.~\ref{tab:optimal_1task} shows that value for completion of one task, whereas Tab.~\ref{tab:optimal_2task} shows the value for completion two tasks. Note that the size being 5 means that there is no hostile agent, which degenerates to the MDP modelling. It is also interesting to see that for the coalition consisting of one agent, it has a positive probability to accomplish one task, but cannot accomplish two tasks for all four organizations.
...
\end{tabular}
\caption{Optimal coalitions of sizes 2, 3, and 4 for agent organisations from Fig.~\ref{fig:network_configurations} for both online and offline versions.}
