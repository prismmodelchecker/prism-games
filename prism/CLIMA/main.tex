...
As is typical for multi-agent algorithms several aspects of the algorithm are modelled using probabilities. Following approach of
\cite{gaston2005agent} we schedule agents to act in a uniformly random order, second probabilistic aspect of the system is that tasks are drawn from a task pool using a probability distribution. This aspect is particularly interesting in the case of \textsc{Online} version of the
 algorithm where agents have to choose strategies to optimise against the probability distribution rather than a particular task. Finally, probabilities are used to implement strategies of agents themselves. This motivates the use of \emph{probabilistic verification} techniques.
