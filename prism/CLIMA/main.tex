...
In Algorithm \ref{alg:join_team_org} we reproduce the pseudocode of the \textsc{JoinTeam} algorithm introduced in \cite{gaston2005agent}. %\textsc{JoinTeam} algorithm is executed separately on each agent whenever it is called.  
...
\comment{Discuss reward structures, topology differences, gap between min and max agent performance. discuss task completion probabilitites, write PCTL formulae. Reference where models can be checker online.}
