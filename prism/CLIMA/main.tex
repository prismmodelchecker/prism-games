...
\label{sec:results}
\vspace{-10pt}
In this section we present results obtained using three models: DTMC, MDP, and STPG. Tab.~\ref{tab:model_sizes} compares model construction information for different sizes of fully connected agent organisations \footnote{We chose fully connected agent organisation because it produces largest models.}. All experiments were performed on a 2.8GHz Intel Core 2 PC, 4Gb of RAM running Fedora Core 13 operating system. Nondeterministic models have a relatively smaller state space because agent choices do not have to be resolved at a model construction stage. However, the model checking is generally more time consuming for MDPs and STPGs than for DTMCs. The time taken to verify PCTL formulae that we used was in the range of 5-240 seconds.
 \vspace{-20pt}
\begin{table}
\centering
\subfloat[DTMC]{
