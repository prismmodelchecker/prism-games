...
%Markov Decision Processes (MDP) (also called Probabilistic Nondetermin-
%istic Systems (PNS) [7]) allow accurate modelling of systems which exhibit
%both probabilistic and nondeterministic behavior. A common example of this
%is the interleaved parallel composition of several probabilistic processes, but
%nondeterminism can also be useful to leave parts of a system underspecified
%and to model interaction with an unknown environment. Formally:
%Definition 5.1 A Markov decision process (MDP) is a tuple (S, s,Act, Steps)
%where S is a finite set of states, s is the initial state, Act is a set of actions
%and Steps  S ¡Á Act ¡Á Dist(S) is a probabilistic transition relation.
%An MDP transition s
%a,¦Ì
