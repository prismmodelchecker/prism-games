...
\begin{itemize}
 \item Fully connected (figure \ref{subfig:fully_connected}) $O_{fc}=\langle A, N_{fc}, R, R_{A_{fc}}  \rangle$ where
    ...
    \item $R_{A_{fc}}=\{R_{a_1}, R_{a_2}, R_{a_3}, R_{a_4}, R_{a_5}\}$ where $R_{a_1}=\{r_1\}$, $R_{a_2}=\{r_1\}$, $R_{a_3}=\{r_1\}$, $R_{a_4}=\{r_1\}$, and $R_{a_5}=\{r_1\}$.
    \end{itemize}
 \item Ring (figure \ref{subfig:ring}) $O_r=\langle A, N_r, R, R_{A_r}  \rangle$ where
    ...
    \item $R_{A_r}=\{R_{a_1}, R_{a_2}, R_{a_3}, R_{a_4}, R_{a_5}\}$ where $R_{a_1}=\{r_1\}$, $R_{a_2}=\{r_1\}$, $R_{a_3}=\{r_1\}$, $R_{a_4}=\{r_1\}$, and $R_{a_5}=\{r_1\}$.
    \end{itemize}
 \item Star (figure \ref{subfig:star}) $O_s=\langle A, N_s, R, R_{A_s}  \rangle$ where
    \begin{itemize}
    \item $N_s=\{\{a_i,a_j \}\ : 1 \le i,j \le 5, i\neq j, i=1 \}$,
    \item $R_{A_s}=\{R_{a_1}, R_{a_2}, R_{a_3}, R_{a_4}, R_{a_5}\}$ where $R_{a_1}=\{r_1\}$, $R_{a_2}=\{r_1\}$, $R_{a_3}=\{r_1\}$, $R_{a_4}=\{r_1\}$, and $R_{a_5}=\{r_1\}$.
    \end{itemize}
 \item Isolated agent (figure \ref{subfig:isolated}) $O_{ia}=\langle A, N_{ia}, R, R_{A_{ia}}  \rangle$ where
    ...
    \item $R_{A_{ia}}=\{R_{a_1}, R_{a_2}, R_{a_3}, R_{a_4}, R_{a_5}\}$ where $R_{a_1}=\{r_1\}$, $R_{a_2}=\{r_1\}$, $R_{a_3}=\{r_1\}$, $R_{a_4}=\{r_1\}$, and $R_{a_5}=\{r_1\}$.
    \end{itemize}
\end{itemize}
For simplicity we assign one resource to each agent. Resource allocation for each of the agent organisations has been chosen such than it is optimal with respect to algorithm \ref{alg:join_team_org} and equation \ref{eq:w2organisation}. We chose this criterion because it measures the total number of tasks performed by organisation. Allocations were computed using DTMC model checking in PRISM (see section \ref{subsection:DTMC} for discussion) assuming the task distribution described below.
...
\subfloat[Fully connected]{
  \begin{tikzpicture}
	  ...
	  -- ++(-180:2.0cm) node (agent4) [label=210:$\LD a_4\ r_1 \RD$] {}
	  -- ++(108:2.0cm) node (agent5) [label=left:$\LD a_5\ r_1 \RD$] {}
	  -- (agent1);
      \draw (agent1) -- (agent3);
      \draw (agent1) -- (agent4);
      \draw (agent2) -- (agent4);
...
\caption{Experimental configurations of the agent organisations.}
...
\begin{table}
 \centering
    ...
    \hline
    Agents & States & Transitions & Verification Time \\ \hline
    1 & 11C & 22C & A clear  \\ \hline
    2 & 9C & 19C & Cloudy  \\ \hline
    3 & 10C & 21C & Rain \\ \hline
    4 & 10C & 21C & Rain \\ \hline
    5 & 10C & 21C & Rain \\ \hline
    6 & 10C & 21C & Rain\\ \hline
\end{tabular}

\caption{DTMC model comparison for different number of agents in fully connected agent organisation. Verification times are for PCTL formula P$_{=?}$ [F ``tasks 1 and 2 completed'']}
\end{table}
