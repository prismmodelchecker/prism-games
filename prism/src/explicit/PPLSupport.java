...
/**
 * contains several methods to augment the Parma Polyhedra Library
...
public class PPLSupport
{
	...
	public static List<Double> getGeneratorAsVector(Generator g, int n) throws PrismException
	{
		...
		for(int i = 0; i < n; i++) // initialise with zeros
			result.add(0.0);
	...
	{
		int n = x.length;
		if(n == 0) throw new PrismException("Generator cannot have zero dimensions.");
	...
	{
	        if(Xs.is_empty()) return false; // an empty polyhedron never satisfies any bound
	...
	{
	        if(polyhedra == null || polyhedra.length == 0) return;
		...
		for (int t = 0; t < polyhedra.length; t++) {
			...
			if(pt != null && pt.size() > 0) {
				...
				if(pt.get(u).is_empty()) continue inner;
		...
		if(mainLog!=null) {
			...
			if(ps.size()>1) System.out.printf("[\n");
			for(int u = 0; u < ps.size(); u++)
			    System.out.printf("%s\n", reachabilityPolyhedronToString(ps.get(u), dim));
			if(ps.size()>1) System.out.printf("]\n");
		    }
