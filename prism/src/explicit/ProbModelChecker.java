...
public class ProbModelChecker extends StateModelChecker {
	...
	private StateValues checkExpressionPATL(Model model, ExpressionPATL expr)
		...
		if (type == ExpressionPATL.PRB) {
			...
			if (!exact) {
				...
				double[] res = ((SMGModelChecker) this).computeUntilProbs(
						(STPG) model, null, terminal, true, true, 1.0).soln;
				...
				double[] minmax = ((SMGModelChecker) this).checkProbPathFormula(model,
						expr, true).getDoubleArray();
				double[] maxmin = ((SMGModelChecker) this).checkProbPathFormula(model,
						expr, false).getDoubleArray();;
	 ...
	 */
	private BitSet findTerminalStates(Model model) {
