...
public class ConstructModel
{
  ...
  public List<State> computeReachableStates(ModulesFile modulesFile,
      Values initialState) throws PrismException
  {
   ...
   * @param initialState The initial state (for reachability)
   */
  public Model constructModel(ModulesFile modulesFile, Values initialState)
   ...
   */
  public Model constructModel(ModulesFile modulesFile, Values initialState,
      boolean justReach, boolean buildSparse) throws PrismException
  {
    // Model info
    ModelType modelType;
    // State storage
    ...
    while (!explore.isEmpty()) {
      ...
      for (i = 0; i < nc; i++) {
        if (!justReach
            && (modelType == ModelType.MDP || modelType == ModelType.STPG)) {
    ...
    if (!justReach && fixdl) {
      ...
      if (deadlocks.cardinality() > 0) {
        mainLog.println("Added self-loops in " + deadlocks.cardinality()
            + " states...");
      }
    }
    ...
    if (!justReach) {
      switch (modelType) {
        case DTMC:
          ...
          ((ModelSimple) model).constantValues = new Values(modulesFile
              .getConstantValues());
          break;
        case CTMC:
        ...
        case MDP:
          if (buildSparse) {
            ...
            ((ModelSparse) model).constantValues = new Values(modulesFile
                .getConstantValues());
          }
          else {
  ...
  private int getPlayer(String actionLabel) throws PrismException
  {
    ...
    if (actionLabel.startsWith("[")) {
      ...
      if (player1mods.contains(actionLabel))
        ...
        throw new PrismException("Module '" + actionLabel
            + "' is not assigned to any player.");
    }
    return player;
  }
  ...
  public static void main(String[] args)
  {
    ...
    try {
      ...
      Model model = constructModel.constructModel(modulesFile, modulesFile
          .getInitialValues());
      MDP mdp = (MDP) model;
      mainLog.println(mdp);
