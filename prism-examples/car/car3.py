...
smg.write("global state : [0..%i] init 0;\n\n" % (M))
...
for i, succs in successors.iteritems():
    ...
    # pick speed - good guy
    smg.write("\t[move_slow_%i] p=1 & s=%i -> (s'=1) & (p'=2);\n" % (k, s))
    s = 1

    # pick available hazards - stochastic guy
    smg.write("\t[hazard_%i] p=2 & s=%i -> " % (k, s))
    init = True
    for j in xrange(0,len(haz)):
        if not init:
            smg.write(" + ")
        init = False
        smg.write("1/%i : (s'=%i) & (p'=2)" % (len(haz), s+1+j))
    smg.write(";\n")
    s = s + len(haz)
